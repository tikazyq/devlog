...
/**
 * New MCP Server with flexible storage architecture
 * Backward compatible with existing tools but uses new storage providers
 */
...
import { NewMCPDevlogAdapter } from "./new-mcp-adapter.js";
...
const adapter = new NewMCPDevlogAdapter(process.cwd());
...
async function main() {
  ...
  console.error("Devlog MCP Server v3 started with flexible storage architecture");
}
