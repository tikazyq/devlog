import { DevlogEntry, AIContext, DevlogContext, Decision, Risk } from "@devlog/types";
...
## Decisions Made
${entry.context.decisions.map(d => 
  `- **${d.decision}** (by ${d.decisionMaker}): ${d.rationale}`
).join('\n')}
...
`;
  ...
  async demonstrateAIWorkflow() {
    ...
    let taskEntry = await this.createTaskWithContext({
      ...
      businessContext: "We need to secure our API endpoints and provide different access levels for admin vs regular users. This is blocking the customer portal launch.",
      technicalContext: "Using Node.js/Express backend with PostgreSQL. Need to integrate with existing user table. Consider using passport.js or implementing custom JWT middleware.",
      acceptanceCriteria: [
    ...
    taskEntry = await this.updateAIUnderstanding(taskEntry, {
      progressSummary: "Analyzed existing user table schema. Designed JWT token structure with roles. Started implementing authentication middleware.",
      newInsights: [
    ...
    taskEntry = await this.recordDecision(taskEntry, {
      ...
      rationale: "Balance between security and performance. 12 rounds provides strong security while keeping response times under 100ms on our server specs.",
      alternatives: [
